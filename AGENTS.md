...
1. Use the Edit tool to make the necessary changes to the text, code, or other
   content within the file.
1. After making changes, use the Bash tool to preview the site with `quarto preview` or `quarto render` to ensure changes are rendered correctly.
...
1. Add the content to the new file using Markdown.
1. If the new page should appear in the site navigation, use the Edit tool to
   add a link to it in the `navbar` section of the `_quarto.yml` file.
1. Use the Bash tool to preview the site to verify that the new page is
...
1. Use the Write tool to create a new `index.qmd` file for the slide deck
   ...
   directory.
1. Add the following YAML header to the top of the file to specify the
   `revealjs` format:
...
3. Use Markdown headings to create slides. A level 2 heading (`##`) creates a
   new slide.
1. Add content (text, images, code blocks) to each slide.
1. Use the Bash tool with `quarto preview` on the specific `.qmd` file to see
   the rendered slides.
1. Slides should always contain suitable icons that help to illustrate a key
   point. Here is an example of the approach to creating the icons: `{{< iconify fa6-solid lightbulb >}}`.
1. Make all the points in the slides short and clear, and easy to read.
1. Do not write points in the slides are long sentences.
1. Always provide evidence to support the points that you make in the slides.
1. Use `fragment` and \`incremental inside of the slides to ensure that content
   displays appropriately through the use of Quarto and the reveal.js framework.
1. Add source code segments using fenced code blocks. Here is an example of what
...
01. Use bold and italics to draw out labels or key take-home messages.
01. The title for a slide should normally fit into a single line. When
...
01. All source code words should be displayed using single backticks
    as in a variable name like `display_name` or `input_list`.
01. The slides should have an appropriate balance of text and code segments
    and a layout that is both predictable and consistent but also varied
    enough to be interesting and engaging.
01. Sometimes the text at the bottom of a slide should use:
...
01. If the code segment is too long and it will require scrolling across
    a lengthy code block, then break up the code block into multiple slides.
01. Make sure that any slide with a code block includes enough space to see
    most, if not all of, the output produced by the source code block.
01. When a slide contains mathematical notation, like P(I), then you should
...
01. Make sure that the slides that you generate closely follow the specified
...
01. Make sure that when you include images, you look for them in the
...
## Agent-Specific Guidelines for OpenCode
...
- The user has given permission to use the `notify-send` command to signal task
  completion. Here is an example of the command: `notify-send "Question from OpenCode" "Please clarify how to complete the writing task."`.
- You also have permission to use the command: `zellij pipe -- "zjstatus::notify::󰵰 Agent finished. <Summarize task description>. "`
- The user wants a `notify-send` and the `zellij` notification whenever I ask a
  question or finish a task.
