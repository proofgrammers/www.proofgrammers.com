...
any current task
- {{< iconify fa6-solid question-circle >}} **Key question**: Does multitasking
...
- Use example to for claim about limits of nondeterminism
...
- {{< iconify fa6-solid x >}} All leaves negative: output of overall program is is `no`
...
|-----------------------|-----------|-------------------------------------------------------------|
| `xATGTTx`             | Yes       | `G` with exactly one `T` between it and an end              |
| `xATTGTGCCTACx`       | Yes       | At least one `G` with exactly one `T` between it and an end |
| `xTGGGTTx`            | Yes       | `G` with exactly one `T` between it and an end              |
| `xATTGx`              | No        | No `G` with exactly one `T` between it and an end           |
| `xCTTGAGTGTATx`       | No        | No `G` with exactly one `T` between it and an end           |
| `xCTCTx`              | No        | No `G` present                                              |
...
- **Concepts like to multithreading in many programming languages**!

:::

## Claim 8.3: equivalence of models

::: {.fragment style="margin-top: 0.1em; font-size: 0.95em;"}
...
- **Implications of this claim**:
  - Nondeterministic Turing machines are more "expressive" than deterministic
    Turing machines
...
non-deterministically, hard deterministically
