...
# Understanding Tractability Beyond Its Definition

- **Simple Search (`search_documents`)**
  - Scans through each document, one by one
  - Finds matches with `if query in doc`
  - Easy to understand, but less efficient as data grows  

- **Improved Search (`Index` class)**
  - Builds an **inverted index**: maps each word → the documents containing it  
