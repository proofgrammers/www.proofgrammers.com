...
class Graph:
  # Defines a graph data type
  def __init__(self: Graph, size: int) -> None:
    self.adj_matrix = [[0] * size for _ in range(size)]
    self.size = size
    self.vertex_data = [''] * size

  # Adds an edge to the graph
  def add_edge(self: Graph, u: int, v: int, weight: int) -> None:
    if 0 <= u < self.size and 0 <= v < self.size:
      self.adj_matrix[u][v] = weight
      #self.adj_matrix[v][u] = weight  # For undirected graph

  # Adds a vertex to the graph
  def add_vertex_data(self: Graph, vertex: int, data: str) -> None:
    if 0 <= vertex < self.size:
      self.vertex_data[vertex] = data

  # Runs the Bellman-Ford Algorithm
  def bellman_ford(self: Graph, start_vertex_data: str) -> arr[int][int]:
...
# Defines current graph with a size of 5
g = Graph(5)

# Adds vertices for the graph (5 vertices here)
...
# Adds edges for the graph between all vertices
