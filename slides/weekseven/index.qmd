...
## Python programs executing other programs
...
- {{< iconify fa6-solid cog >}} **Three-step process**
...
::: {.fragment .fade .boxed-content style="font-size: 1.0em;"}
**Progression**: From string analysis to program simulation to program alteration
:::
...
def repeat_ca_or_ga(in_string):
    """Original program that returns specific patterns."""
    if in_string == 'CA':
        return 'CACA'
    elif in_string == 'GA':
        return 'GAGA'
...
def alter_gaga_to_tata(encoded_input):
    """Alters programs: changes GAGA output to TATA."""
    # In practice, would decode prog_string and new_in_string
    # For demo: simulate the alteration
    original_result = repeat_ca_or_ga('GA')
    print(f"Original result: {original_result}")
    
    # The crucial alteration
    if original_result == 'GAGA':
        return 'TATA'
    else:
        return original_result

# Demonstrate program alteration
result = alter_gaga_to_tata("dummy_input")
print(f"Altered result: {result}")
```

:::

::: {.fragment .fade .boxed-content style="font-size: 1.0em;"}
**Power**: One program can modify and execute any other program
:::

## The strange `ignoreInput` program
...
::: {.fragment .fade .boxed-content style="font-size: 1.0em;"}
**Purpose**: Fixed computation regardless of input - crucial for proofs!
:::
...
## Rule 110 cellular automaton visualization
...
Each row represents one computational step
Each cell's next state depends on its three neighbors above
Simple rules create complex, universal computational behavior

:::

::: {.fragment .fade .boxed-content style="font-size: 1.0em;"}
**Amazing**: 8 simple rules = unlimited computational power
:::

## Recognizable vs decidable problems
...
#| autorun: true
#| max-lines: 12
def rec_yes_on_string(encoded_input):
    """Recognizes YESONSTRING - may not terminate on negatives."""
    # Simulate decoding (prog_string, new_in_string)
    prog_string = "def test(s): return 'yes'"
    input_string = "test_input"
    
    # Simulate universal execution
    namespace = {}
    exec(prog_string, namespace)
    result = namespace['test'](input_string)
    
    if result == 'yes':
        return 'yes'
    else:
        return 'no'  # Or might loop forever in practice

print(rec_yes_on_string("dummy"))
```
