...
always including blank symbol $\sqcup$
...
- {{< iconify fa6-solid dna >}} **Examples**:
  ...
  - CGAT → CGAA
  - TTTT → TTTA
  - *Wait*, what would happen with GGGG?
...
::: {.fragment .fade style="font-size: 0.675em;"}
...
- Next must define functions to run the machine for an input
...
def run_machine(tm, input_string, max_steps=100):
    ...
    print(f"Final: {tm.state}: {format_configuration(tm)} / Output: {output}")
    return output
...
- {{< iconify fa6-solid input-output >}} **Transducer Turing machines:**  
    ...
    - Like a SISO program that returns a string

- {{< iconify fa6-solid check-circle >}} **Acceptor Turning machines:**  
