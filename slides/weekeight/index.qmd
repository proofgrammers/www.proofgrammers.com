...
# How do we prove a computational problem is hard?
...
- {{< iconify fa6-solid gear >}} **Two proof approaches we've seen**
    - **Direct proof**: Diagonalization argument (like halting problem)
    - **Reduction proof**: Transform known hard problem to new problem

:::

## Reduction is the focus today

::: {.incremental style="margin-top: -0.5em; font-size: 0.85em;"}

- {{< iconify fa6-solid arrow-right >}} **Key insight**: If we can solve new problem, then we can solve known uncomputable problem
- {{< iconify fa6-solid arrow-right >}} **Contradiction**: Known problem is uncomputable by previous proof
- {{< iconify fa6-solid arrow-right >}} **Conclusion**: New problem must also be uncomputable
...
- {{< iconify fa6-solid definition >}} **Formal definition**: 
    ...
    - We write this as $F \leq_T G$ (read "$F$ reduces to $G$")

:::

::: {.fragment style="margin-top: -0.5em; font-size: 0.85em;"}

- {{< iconify fa6-solid lightbulb >}} **Intuitive meaning**:
...
## Oracle programs make reductions work

```{pyodide}
#| autorun: true
def solve_F_using_G_oracle(input_to_F):
    """Solve problem F by calling oracle for problem G."""
    # Transform input for F into input for G
    transformed_input = transform_for_G(input_to_F)
    
    # Call mythical oracle that solves G
    g_result = G_oracle(transformed_input)
    
    # Transform G's result back to F's format
    return transform_result_for_F(g_result)

def transform_for_G(f_input):
    """Transform F input to G input format."""
    return f"transformed_{f_input}"

def G_oracle(g_input):
    """Mythical oracle that solves problem G."""
    return "oracle_solution"

def transform_result_for_F(g_result):
    """Transform G result to F result format."""
    return f"F_answer_from_{g_result}"

# Demonstrate the reduction pattern
result = solve_F_using_G_oracle("test_input")
print(f"F result: {result}")
```

::: {.fragment style="font-size: 0.9em;"}

{{< iconify fa6-solid key >}} **Key point**: Oracle for $G$ may not exist, but we assume it does to build our reduction

:::

# Example: Reducing `YESONSTRING` to `CONTAINSGAGA`

::: {.incremental style="margin-top: -0.5em; font-size: 0.85em;"}

- {{< iconify fa6-solid target >}} **Goal**: Prove `CONTAINSGAGA` is uncomputable
- {{< iconify fa6-solid known >}} **Known fact**: `YESONSTRING` is uncomputable (Chapter 3)
...
- {{< iconify fa6-solid gear >}} **Problem definitions**:
    ...
    - `CONTAINSGAGA`: Given program $P$ and input $I$, does $P(I)$ contain "GAGA"?

:::

## Step 1: Transform programs

```{pyodide}
#| autorun: true
def alter_yes_to_gaga(prog_string, input_string):
    """Transform program to output GAGA instead of yes."""
    def new_program(input_data):
        # Simulate running original program
        original_result = simulate_program(prog_string, input_string)
        
        # Transform output: yes becomes GAGA, anything else becomes empty
        if original_result == "yes":
            return "GAGA"
        else:
            return ""
...
def simulate_program(prog, inp):
    """Simulate program execution (simplified)."""
    # In reality, this would run the actual program
    return "yes"  # Pretend result for demonstration

# Test the transformation
transformed = alter_yes_to_gaga("sample_prog", "sample_input")
result = transformed("any_input")
print(f"Transformed program output: '{result}'")
```

::: {.fragment style="font-size: 0.9em;"}

{{< iconify fa6-solid check >}} **Step 1 complete**: New program outputs "GAGA" iff original outputs "yes"

:::

## Step 2: Complete the reduction

```{pyodide}
#| autorun: true
def solve_yesonstring_via_containsgaga(prog_string, input_string):
    """Solve YESONSTRING by reducing to CONTAINSGAGA."""
    # Create transformed program that outputs GAGA instead of yes
    transformed_prog = alter_yes_to_gaga(prog_string, input_string)
    
    # Convert transformed program to string representation
    transformed_prog_string = "altered_program_code"
    
    # Call CONTAINSGAGA oracle on transformed program
    contains_gaga_result = containsgaga_oracle(transformed_prog_string, input_string)
    
    # Return the same result (yes iff contains GAGA)
    return contains_gaga_result

def containsgaga_oracle(prog_str, inp_str):
    """Oracle for CONTAINSGAGA (mythical but assumed to exist)."""
    print("Oracle determines if program output contains GAGA")
    return "yes"  # Pretend oracle result

# Demonstrate complete reduction
print("Solving YESONSTRING via CONTAINSGAGA:")
result = solve_yesonstring_via_containsgaga("test_prog", "test_input")
print(f"YESONSTRING result: {result}")
```

::: {.fragment style="font-size: 0.9em;"}

{{< iconify fa6-solid trophy >}} **Reduction complete**: If `CONTAINSGAGA` oracle exists, we can solve `YESONSTRING`

:::

# Why this proves `CONTAINSGAGA` is uncomputable

::: {.incremental style="margin-top: -0.5em; font-size: 0.85em;"}

- {{< iconify fa6-solid step1 >}} **Step 1**: Assume `CONTAINSGAGA` is computable
- {{< iconify fa6-solid step2 >}} **Step 2**: Then `YESONSTRING` is computable (via our reduction)
- {{< iconify fa6-solid step3 >}} **Step 3**: But `YESONSTRING` is uncomputable (proven in Chapter 3)
- {{< iconify fa6-solid contradiction >}} **Contradiction**: Cannot have both computable and uncomputable
