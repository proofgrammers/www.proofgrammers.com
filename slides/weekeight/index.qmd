...
![](07-reductions_0.png){.sensible-size-thick-border}
...
::: {.incremental style="margin-top: -0.5em; font-size: 0.805em;"}
...
::: {.fragment .boxed-content style="margin-top: -0.25em; font-size: 0.715em;"}
...
- **Key**: Importantly, all of these uncomputability proofs start from a single 
...
![](07-reductions_5.png){ .sensible-size-thick-border }
...
- {{< iconify fa6-solid lightbulb >}} Can we prove that `HaltsOnString` is
undecidable?
...
$\leq_T$ `HaltsOnString`. Next, what is the implementation of `haltsOnString`?
...
- This approximate version uses `runWithTimeout` to simulate halting behavior
since the `universal` program may run forever with programs having infinite
loops
...
- The `alterYesToHalt` program returns `'halted'` if $P(I)=$`'yes'` and
otherwise enters an infinite loop using the utility function `utils.loop()`
function
...
- `yesViaHalts`: uses the `haltsOnString` oracle to decide `YesOnString` by
  calling the oracle on the transformed program.
    - `yesViaHalts` is functionally equivalent to `YesOnString`
    - However, `yesViaHalts` uses `haltsOnString` as a key building block

- `alterYesToHalt.py`: transforms an instance `(P, I)` so that if `P(I)`
  returns `"yes"` the transformed program halts (returns), otherwise it enters
  an infinite loop.

- `haltsOnString`: oracle wrapper that simulates a program with timeout and
  returns `"yes"` iff the program halts within the timeout period.
...
![](07-reductions_17.png){ .sensible-square-thick-border height=95% }
