...
zero or more times
- {{< iconify fa6-solid box >}} **Example**: "TGACGT" → pump "AC" →
"TGACACGT" or "TGT"
- {{< iconify fa6-solid circle-check >}} **Still in language**: pumped
string remains valid
- {{< iconify fa6-solid microscope >}} **Insight**: regular
...
**Pumping exploits finite-state limitation**: if a DFA accepts a long string, it
must revisit states, creating a cycle that can be repeated or skipped. This
forced repetition is the essence of pumping, helps in proof by contradiction.
...
- {{< iconify fa6-solid lightbulb >}} **Critically, all regular languages have
this property!**
...
![Define the concept of pumping a language $L$](09-finite-automata_34.png)
...
- **Key**: Instead of using fact that "regular languages can be pumped," we use
the equivalent statement "if it can’t be pumped, then it’s not regular."

:::

## Proving non-regularity of language

::: {.incremental .fade style="font-size: 0.725em;"}

- **Proof by contradiction**: the pumping lemma is a powerful tool. To
prove $L$ is not regular, assume that it is and show that no matter how we pump,
we eventually generate a string not in $L$. This contradicts regularity and
proves $L$ is not regular.
...
- {{< iconify fa6-solid lightbulb >}} **Proof exercise: try to construct DFAs
for operations!**
...
- **Proving non-regularity**: if $L$ regular and $\overline{L}$ is not,
contradiction!
...
**Finite automata reveal computational limits**: despite their simplicity and
practical utility, they cannot recognize all decidable languages.
