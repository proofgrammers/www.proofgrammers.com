...
- {{< iconify fa6-solid map-location-dot >}} **Instance $I$**: Graph
`"a,b,1 b,c,2 c,a,3;8"` with threshold $L = 8$
- {{< iconify fa6-solid route >}} **Hamilton cycle**: Path visiting each
vertex exactly once and returning to start
- {{< iconify fa6-solid scale-balanced >}} **TSPD decision problem**: Asks
"yes/no" whether a Hamilton cycle of length $\leq L$ exists
- {{< iconify fa6-solid question >}} **Question**: Does a Hamilton cycle of
length $\leq 8$ exist?
- {{< iconify fa6-solid check-check >}} **Proposed solution $S$**: `"yes"`
- {{< iconify fa6-solid lightbulb >}} **Hint $H$**: `"a,b,c"` (the actual
Hamilton cycle)
- {{< iconify fa6-solid link >}} **Why hint helps**: TSPD only asks yes/no,
but verifying requires seeing the actual cycle
- {{< iconify fa6-solid calculator >}} **Verification**: Check cycle visits
all vertices once
- {{< iconify fa6-solid ruler >}} **Check length**: $1 + 2 + 3 = 6 \leq 8$
- {{< iconify fa6-solid rocket >}} **Result**: `"correct"` in $O(n^2)$ time!

:::

::: {.fragment .fade .boxed-content style="font-size: 0.75em;"}

**Without the hint, we'd need exponential time**: there are $n!$ possible cycles
to check! The hint `"a,b,c"` tells us exactly which cycle to verify, reducing
an exponential search to a polynomial-time check. This is the power of hints!
