...
## Tractable vs Intracable vs Uncomputable (Part 2)

::: {.incremental style="margin-top: -0.5em; font-size: 0.8em;"}
- Tractable (in restricted cases)
    - runs in polynomial time
        - "The complexity class _Poly_ is the set of computational problems can be solved by a Python program with running time in $O(nᵏ)$, for some $k≥0$."
    - ![Complexity Proof](Screenshot%2025-09-08%100559.png)
...
- If you change the inequality of program P1 from $toal > 10$ to $10 < toal$ you would now get a negative even though the programs are still equivelent
...
- There are an infinte number of ways to structure a program that will get the same results so it's unfeasible to make workarounds for structures being differnt but equivelent.
- It's not feasible to run every posible input through both programs as it would take an infinte amount of time to get through an infinte amount of inputs.
...
- This approach only works for restricted cases, in the general case determining program equivalence is incomputable.
...
- This challenge demonstrates that while tools can help with specific cases, fully determining program equivalence for all programs is incomputable.
:::