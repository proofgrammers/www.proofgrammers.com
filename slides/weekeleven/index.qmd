...
time
- {{< iconify fa6-solid times >}} **Combined complexity**: $O(n²)$
iterations × $O(log n)$ per iteration = **O(n² log n)**
...
$O(log 1000) = O(3)$ time to convert to string. For input
size $n$, indices can reach $n$, requiring $O(log n)$ time per
conversion. This logarithmic factor multiplies the quadratic loop structure,
