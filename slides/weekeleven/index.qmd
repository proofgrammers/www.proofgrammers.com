...
objectives **CS-204-1** and **CS-204-4**
- Distinguish tractable from intractable problems
- Classify problems by complexity class
- **Key questions**: Why are some problems efficiently solvable while
others require impractical time?
...
- Uncomputability showed absolute limits
- Complexity reveals practical limits
- Exponential-time algorithms may work theoretically but are unusable for
large inputs
...
- Now we study time restrictions: how quickly can problems be solved?
...
- **Automata theory connects to complexity theory**
- Finite automata solve their problems efficiently (linear time)
...
- {{< iconify fa6-solid balance-scale >}} **Polynomial vs exponential**:
...
- Algorithm taking $100n$ steps is fundamentally faster than $n^2$ for
large $n$
- Enables meaningful comparisons across machines and implementations
...
boundary defines tractability**
- Polynomial-time problems are considered efficiently computable
- Exponential-time problems are generally impractical
...
- {{< iconify fa6-solid function >}} **Upper bound**: Big-O provides a
formal upper bound on growth rate
- {{< iconify fa6-solid filter >}} **Dominant terms**: Keep only the
fastest growing term, drop constants
- {{< iconify fa6-solid graduation-cap >}} **Standard language**: Common
notation for discussing algorithm efficiency
- {{< iconify fa6-solid exclamation-triangle >}} **Common mistakes**:
Input size vs value, arithmetic operation costs
...
- No need to worry about implementation details or hardware differences
...
- **Formal definition**: $f(n) = O(g(n))$ means constants $c$ and $n_0$
exist
- Such that $f(n) \leq c \cdot g(n)$ for all $n \geq n_0$
- Captures intuition that $f$ grows no faster than $g$ (up to constant
factors)
...
complexity analysis**
- Always distinguish input size from value
...
programs to determine complexity
- {{< iconify fa6-solid repeat >}} **Loop analysis**: Nested loops often
indicate quadratic or higher complexity
- {{< iconify fa6-solid search >}} **String operations**: Common
operations like substring search have linear time
- {{< iconify fa6-solid robot >}} **Turing machines**: Formal running
time analysis extends to theoretical models

:::

::: {.fragment .fade .boxed-content style="font-size: 0.8em;"}

**Running time analysis reveals algorithm efficiency**: count how many
times the innermost operations execute as a function of input size $n$.
This skill helps proofgrammers write faster, more scalable programs!
...
- Comparison is $O(1)$
- Total: $O(n \times n) = O(n^2)$ quadratic time
...
- For input length $n$: $O(n)$ linear time
- Much faster than $O(n^2)$ for large inputs
