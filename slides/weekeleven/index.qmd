...
::: {.fragment .fade .boxed-content style="font-size: 0.625em;"}

- {{< iconify fa6-solid lightbulb >}} **String operations are not free**: The
integer 999 has 3 digits, requiring $O(log 1000) = O(3)$ time
to convert to string. For input size $n$, indices can reach $n$, requiring
$O(log n)$ time per conversion. This logarithmic factor multiplies the
quadratic loop structure, yielding $O(n² log n)$ total complexity!
