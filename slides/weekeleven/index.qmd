...
**The nested loops suggest O(n²) complexity**: With input `'aaa aaaaa'`, the
function performs $3 \times 5 = 15$ iterations. However, the complete analysis
requires examining what happens *inside* each iteration, particularly the
string construction operations!
...
- {{< iconify fa6-solid text-width >}} **String construction cost**: Line 10
creates `thisPair = str(i) + ',' + str(j)`
- {{< iconify fa6-solid hashtag >}} **Integer-to-string conversion**:
Converting integer `i` or `j` to string requires O(log n) time
- {{< iconify fa6-solid list-ol >}} **Why logarithmic?**: Number of digits in
integer `k` is O(log₁₀ k), so `str(k)` takes O(log k) time
- {{< iconify fa6-solid times >}} **Combined complexity**: O(n²) iterations ×
O(log n) per iteration = **O(n² log n)**

:::

::: {.fragment .fade .boxed-content style="font-size: 0.85em;"}

**String operations are not free**: The integer 999 has 3 digits, requiring
O(log 1000) = O(3) time to convert to string. For input size n, indices can
reach n, requiring O(log n) time per conversion. This logarithmic factor
multiplies the quadratic loop structure, yielding O(n² log n) total complexity!
...
- However, this is exponential in $n$!
